...
public class CondorQProcessor extends CondorJobStatusProcessor {
	
	private static final int MY_ARG_LEN = 9;
	
	public CondorQData[] getJobs() {
		CondorQData[] jobData = null;
		try {
		    Process titanJob = Runtime.getRuntime().exec("condor_q -global");
	...
	public CondorQData getJob(String jobID) {
		CondorQData jobData = null;
		try {
		    Process titanJob = Runtime.getRuntime().exec("condor_q -global " + jobID);
		    BufferedReader rs = new BufferedReader(new InputStreamReader(titanJob.getInputStream()));
		    String line;
		    ArrayList<String> jobArray = new ArrayList<String>();
		    while((line = rs.readLine()) != null) {
