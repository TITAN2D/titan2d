...
# $python createVolcanoKML_POLYGONS_LASTFRAME.py /Users/kyle/CCR/Spring2012/google_earth/lasttimestep/ $TOOLDIR/bin
...
merapi = open(outDir+'/pileheight_lastimage.kml', 'w')
...
#imagefile = string.replace(imagefile, "createVolcanoKML_POLYGONS_LASTFRAME.py", "pileheight.png")
...
print "Displaying file: ", fname

### Once we reach the last points file:
if not fname == "":
	...
	heights.close()
        ...
        if exceptCount != 0:
            ...
            print("except count: %d" %exceptCount)
            print("percent: %.4f" %(float(exceptCount)/float(tryCount)))
...
zf = zipfile.ZipFile(outDir+"/pileheightLast.kmz", mode="w")
zf.write(outDir+"/pileheightlabel.png", "pileheight.png")
zf.write(outDir+"/pileheight_lastimage.kml", "pileheight_lastimage.kml")
zf.close()
os.remove(outDir+"/pileheightlabel.png");
os.remove(outDir+"/pileheight_lastimage.kml");
