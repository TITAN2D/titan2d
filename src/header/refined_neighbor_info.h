...
struct refined_neighbor
{
    ...
    void set_parameters(int proc, unsigned* target, unsigned* old, unsigned* new_sons, int* which, int og,
                        int case_flag)
    {
        target_proc = proc;
        orig_gen = og;
    ...
    void set_parameters(int proc, const SFC_Key& target, const SFC_Key& old, const Element* EmTemp, const int * which, int og,
                            int case_flag)
    {
        target_proc = proc;
        orig_gen = og;
        ...
        if(case_flag == 1)
        {
            SET_OLDKEY(sons[0], EmTemp->son(which[1]));
            SET_OLDKEY(sons[1], EmTemp->son(which[0]));
        }
        else if(case_flag == 2)
        {
            SET_OLDKEY(sons[0], EmTemp->son(which[0]));
