...
int load_GDAL_data() {
	...
	if (poBand->GetColorTable() != NULL) {
		cout << "Band has a color table with " << poBand->GetColorTable()->GetColorEntryCount()
		    << " entries." << endl;
	...
	for (int i = 0; i < nrows; i++)
		if (poBand->RasterIO(GF_Read, 0, i, ncols, 1, gis_grid.elev[i], ncols, 1, GDT_Float32, 0, 0)
		    != CE_None) {
			cout << "FATAL ERROR:" << endl;
