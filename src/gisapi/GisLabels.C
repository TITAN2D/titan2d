...
bool GisLabels::getLabel(int nlabel, string& labelString, double& x, double& y) {
	if (nlabel < _nlabels) {
...
int GisLabels::getIndex(string& labelStr) {
...
void GisLabels::print() {
	vector<string>::iterator i;
	vector<double>::iterator j;
	vector<double>::iterator k;
	for (i = _labelStrings.begin(), j = _labelXs.begin(), k = _labelYs.begin();
	    i != _labelStrings.end(), j != _labelXs.end(), k != _labelYs.end(); i++, j++, k++)
		cout << (*i) << " " << (*j) << " " << (*k) << endl;
