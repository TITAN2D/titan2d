...
void GisLine::getPoints(double* xCoords, double* yCoords) {
	int k;
...
bool GisLine::findPoints(double x, double y, double scale) {
...
void GisLine::insertPoints(double x, double y) {
	_xS.push_back(x);
	_yS.push_back(y);
	_nPoints = (int) _xS.size();
}
...
void GisLine::print() {
	...
	for (i = _xS.begin(), j = _yS.begin(); i != _xS.end(), j != _yS.end(); i++, j++)
		cout << "(" << (*i) << " " << (*j) << ") ";
}
...
int GisLines::getLine(int nLine, double* x, double* y) {
	if (nLine < _nLines) {
...
int GisLines::getIndex(int nLine, int* lineIndex) {
	if (nLine < _nLines) {
...
int GisLines::getLabel(int nLine, string* lineStr) {
	if (nLine < _nLines) {
...
int GisLines::getLineSize(int nLine, int* lineSize) {
	if (nLine < _nLines) {
...
void GisLines::setIndices(GisLabels& labels, double scale) {
	//DEUBG
