 ...
 *
 *******************************************************************
 * $Id: GisBinFile.C 233 2012-03-27 18:30:40Z dkumar $ 
 */
...
GisBinFile::GisBinFile(const string& name, const char* mode) :
		fileName_(name), mode_(mode), swappMode_(false), wordSize_(2), isInteger_(true), compressed_(
		    false) {
...
void GisBinFile::setEndian(const char* mode) {
	swappMode_ = true;
...
bool GisBinFile::read(short* shortValue) {
	if (file_.good()) {
		if ((wordSize_ == 2) && isInteger_) {
			if (file_.read((char*) shortValue, wordSize_)) {
				if (swappMode_)
					*shortValue = (((*shortValue >> 8) & 0x00FF) + ((*shortValue << 8) & 0xFF00));
			}
			return true;
		}
	}
...
bool GisBinFile::read4Bytes(char* char4Values) {
	if (swappMode_) {
...
bool GisBinFile::read(int* intValue) {
	if (file_.good())
		if (wordSize_ == 4)
...
bool GisBinFile::read(float* floatValue) {
	if (file_.good())
		if (wordSize_ == 4)
...
bool GisBinFile::read(off_t *offsetVal) {
	char *buf, *b;
	unsigned char c;
	off_t v;
	int i, sz;
...
bool GisBinFile::readNChar(char* charValues, int nValues) {
	if (file_.good())
...
void GisBinFile::gotoPos(long newPos) {
	file_.seekg(newPos);
}
...
bool GisBinFile::readRow(int row, char* charValues) {
	if (file_.good()) {
		this->gotoPos(0L);
		if (this->isCompressed())
...
bool GisUncompress(int inSize, unsigned char* inChars, int outSize, unsigned char* outChars) {
	...
	if (err == Z_OK) {
		...
		if (!(err == Z_STREAM_END || err == Z_OK)) {	//Some error
