...
void GisGrid::setNumRowsCols(int nRows, int nCols)
{
    if(nRows > 0 && nCols > 0)
    {
        nRows_ = nRows;
...
void GisGrid::setNumRows(int nRows)
{
    if(nRows > 0)
    {
        nRows_ = nRows;
...
void GisGrid::setNumCols(int nCols)
{
    if(nCols > 0)
    {
        nCols_ = nCols;
...
double GisGrid::get(int row, int col)
{
...
void GisGrid::setMax(GisTriOut & tri, int simVar)
{
...
void GisGrid::smooth3()
{
    ...
    for(i = 0; i < nRows_; i++)
    {
        ...
        for(j = 0; j < nCols_; j++)
        {
            ...
            fVal = (gisGrid_[k1 + m1] + gisGrid_[k1 + m2] + gisGrid_[k1 + m3] + gisGrid_[k2 + m1] + gisGrid_[k2 + m2]
                    + gisGrid_[k2 + m3] + gisGrid_[k3 + m1] + gisGrid_[k3 + m2] + gisGrid_[k3 + m3])
                   / 9.;
            outBuf.push(fVal);
