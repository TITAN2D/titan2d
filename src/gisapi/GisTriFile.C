...
bool GisTriOut::contains(double x, double y) {
	double totalArea, triangleArea;

//	Calculate the base triangle area 
	triangleArea = fabs(((x1_ - x0_) * (y2_ - y0_)) - ((x2_ - x0_) * (y1_ - y0_)));
	triangleArea *= 1.00001;

	totalArea = fabs(((x0_ - x) * (y1_ - y)) - ((x1_ - x) * (y0_ - y)));
	if (totalArea > triangleArea)
		return false;

	totalArea += fabs(((x1_ - x) * (y2_ - y)) - ((x2_ - x) * (y1_ - y)));
	if (totalArea > triangleArea)
		return false;

	totalArea += fabs(((x0_ - x) * (y2_ - y)) - ((x2_ - x) * (y0_ - y)));
	if (totalArea > triangleArea)
		return false;
...
void GisTriOut::print() {
	cout.precision(12);
	cout << " " << (int) x0_ << " " << (int) y0_ << " " << (int) x1_ << " " << (int) y1_ << " "
	    << (int) x2_ << " " << (int) y2_ << endl;
}
...
bool GisTriFile::readTimeStepInfo() {
	if (this->good()) {
		float floatVal;
		this->read(&numTri_);
		this->read(&timeStep_);
...
bool GisTriFile::readTriData(GisTriOut& triOut) {
	if (this->good()) {
		float floatVal;
