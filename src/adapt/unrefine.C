...
void unrefine_interp_neigh_update(HashTable* El_Table, HashTable* NodeTable, int nump, int myid, void* OPU)
{
    ElemPtrList* OtherProcUpdate = (ElemPtrList*) OPU;
                    ...
                    if(ifrecvd)
                    {
                        ...
                        for(iopu = 0; iopu < num_recv[iproc]; iopu++)
                        {
                            ...
                            if(EmTemp->adapted == OLDSON)
                            {
                                ...
                                EmFather->neigh_gen[ineighmod4] = EmFather->generation;
                                EmFather->neigh_gen[ineighmod4 + 4] = EmFather->generation;
                            ...
                            else if(EmTemp->adapted >= NOTRECADAPTED)
                            {
                                ...
                                EmTemp->neigh_gen[ineighmod4 + 4] = EmTemp->neigh_gen[ineighmod4] - 1;
                                EmTemp->neigh_gen[ineighmod4] = EmTemp->neigh_gen[ineighmod4] - 1;
