...
extern void create_new_node(int, int, int, HashTable*, Node*[], unsigned[][2], int, int*, int, int,
    MatProps*);
...
//+++++++++++++++++++++++++++SIDE1
	...
	Quad9P = new Element(nodes, neigh, neigh_proc, bcptr, generation, elm_loc, &NewOrder[0][0],
	    neigh_gen, material, EmTemp, coord, HT_Elem_Ptr, HT_Node_Ptr, myid, matprops_ptr,
	...
	dpson[0] = *(EmTemp->get_drypoint() + 0) * 2 - 0.5;
	dpson[1] = *(EmTemp->get_drypoint() + 1) * 2 + 0.5;
	Quad9P = new Element(nodes, neigh, neigh_proc, bcptr, generation, my_elm_loc, &NewOrder[1][0],
	    neigh_gen, material, EmTemp, coord, HT_Elem_Ptr, HT_Node_Ptr, myid, matprops_ptr,
	    iwetnodefather, Awetfather, dpson);
	...
	dpson[0] = *(EmTemp->get_drypoint() + 0) * 2 - 0.5;
	dpson[1] = *(EmTemp->get_drypoint() + 1) * 2 - 0.5;
	Quad9P = new Element(nodes, neigh, neigh_proc, bcptr, generation, my_elm_loc, &NewOrder[2][0],
	    neigh_gen, material, EmTemp, coord, HT_Elem_Ptr, HT_Node_Ptr, myid, matprops_ptr,
	    iwetnodefather, Awetfather, dpson);
	state_vars = Quad9P->get_state_vars();
	//printf("state_vars= %g   %g   %g\n",state_vars[0],state_vars[1],state_vars[2]);
	...
	dpson[0] = *(EmTemp->get_drypoint() + 0) * 2 + 0.5;
	dpson[1] = *(EmTemp->get_drypoint() + 1) * 2 - 0.5;
	Quad9P = new Element(nodes, neigh, neigh_proc, bcptr, generation, my_elm_loc, &NewOrder[3][0],
	    neigh_gen, material, EmTemp, coord, HT_Elem_Ptr, HT_Node_Ptr, myid, matprops_ptr,
	    iwetnodefather, Awetfather, dpson);
	state_vars = Quad9P->get_state_vars();
	//printf("state_vars= %g   %g   %g\n\n",state_vars[0],state_vars[1],state_vars[2]);
...
void create_new_node(int which, int Node1, int Node2, HashTable* HT_Node_Ptr, Node* NodeTemp[],
	...
	for (i = 0; i < 2; i++)
		NewNodeCoord[i] = (*(NodeTemp[Node1]->get_coord() + i) + *(NodeTemp[Node2]->get_coord() + i))
		    * .5;
