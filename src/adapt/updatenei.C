...
void ElemBackgroundCheck(HashTable* El_Table, HashTable* NodeTable, unsigned *debugkey, FILE *fp) {
	...
	if (EmDebug) {
		...
		for (inode = 0; inode < 8; inode++) {
			...
			//assert(NdTemp);
			if (NdTemp)
				fprintf(fp, " %d:   {%10u,%10u}  info=%d\n", inode, NdTemp->key[0], NdTemp->key[1],
				    NdTemp->info);
			else
...
void NodeBackgroundCheck(HashTable *El_Table, HashTable* NodeTable, unsigned *nodedbkey, FILE *fp) {
	...
	if (NdDebug)
		fprintf(fp, "NdDebug={%10u,%10u} info=%d belongs to %d elements\n", nodedbkey[0], nodedbkey[1],
		    NdDebug->info, ElemList.get_num_elem());
	else
		fprintf(fp, "NdDebug={%10u,%10u} is missing; it belongs to %d elements\n", nodedbkey[0],
		    nodedbkey[1], ElemList.get_num_elem());
...
int ifCheckNode(HashTable* El_Table, HashTable* NodeTable, int NdDebugInfo, int checkstate,
    unsigned *elemdbkey, Element *EmFather, Element *EmSonA, Element *EmSonB,
	...
	for (ielembucket = 0; ielembucket < El_Table_num_buck; ielembucket++) {
		...
		while (El_Table_entry_ptr) {
			...
			if (EmTemp->get_adapted_flag() >= NOTRECADAPTED) {
				...
				coord = NdTemp->get_coord();
				assert(
				    (coord[0] - tol <= xmin) && (xmin <= coord[0] + tol) && (coord[1] - tol <= ymin)
				        && (ymin <= coord[1] + tol));
				...
				coord = NdTemp->get_coord();
				assert(
				    (coord[0] - tol <= xmax) && (xmax <= coord[0] + tol) && (coord[1] - tol <= ymax)
				        && (ymax <= coord[1] + tol));
				...
				coord = NdTemp->get_coord();
				assert(
				    (coord[0] - tol <= xmean) && (xmean <= coord[0] + tol) && (coord[1] - tol <= ymean)
				        && (ymean <= coord[1] + tol));
...
int IfNeighProcChange(HashTable* El_Table, HashTable* NodeTable, int myid, Element* EmDebug,
    Element* EmTemp) {
	if (myid == 2)
				...
				if ((neigh_proc >= 0) && (neigh_proc != myid)) {
					...
					switch (EmFather->generation - EmFather->neigh_gen[ineigh]) {
						case -1:
							...
							EmSon[isonA]->neigh_gen[ineighm4] = EmSon[isonA]->neigh_gen[ineighp4] =
							...
							if (EmFather->neigh_proc[ineighother] == myid) {
								...
								EmSon[isonB]->neigh_gen[ineighm4] = EmSon[isonB]->neigh_gen[ineighp4] =
								...
								for (ikey = 0; ikey < KEYLENGTH; ikey++)
									EmSon[isonA]->neighbor[ineighm4][ikey] = EmSon[isonA]->neighbor[ineighp4][ikey] =
									    EmSon[isonB]->neighbor[ineighm4][ikey] =
									        EmSon[isonB]->neighbor[ineighp4][ikey] = EmFather->neighbor[ineigh][ikey];

								EmSon[isonA]->neigh_gen[ineighm4] = EmSon[isonA]->neigh_gen[ineighp4] =
								    EmSon[isonB]->neigh_gen[ineighm4] = EmSon[isonB]->neigh_gen[ineighp4] =
								        EmFather->generation;

								EmSon[isonA]->neigh_proc[ineighm4] = EmSon[isonB]->neigh_proc[ineighm4] =
								    neigh_proc;
				...
				if ((neigh_proc >= 0) && (neigh_proc != myid)) {
					...
					for (ikey = 0; ikey < KEYLENGTH; ikey++) {
						...
						send[neigh_proc][(4 * isend[neigh_proc] + 2) * KEYLENGTH + ikey] = send[neigh_proc][(4
						    * isend[neigh_proc] + 3) * KEYLENGTH + ikey] = EmFather->son[isonB][ikey];
					}
					...
					if (ifrecvd) {
						...
						for (irecvd = 0; irecvd < num_recv[iproc]; irecvd++) {
							...
							if (EmTemp->adapted == OLDFATHER) {
								...
								EmTemp->neigh_gen[ineighp4] = EmTemp->neigh_gen[ineigh] = EmTemp->neigh_gen[ineigh]
								    + 1;
...
void update_neighbor_info(HashTable* HT_Elem_Ptr, ElemPtrList* RefinedList, int myid, int nump,
