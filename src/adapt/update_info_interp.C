...
void update_neighbor_interprocessor(HashTable* HT_Elem_Ptr, HashTable* HT_Node_Ptr,
	...
	for (i = 1; i < numprocs; i++) {
		...
		MPI_Isend(&send_counter[mate_proc], 1, MPI_INT, mate_proc, atag, MPI_COMM_WORLD,
		    (request + mate_proc));
		MPI_Isend(send_buffer[mate_proc], send_counter[mate_proc], REFINED_INFO, mate_proc, refinedtag,
		    MPI_COMM_WORLD, (request2 + mate_proc));
		...
		MPI_Irecv(recv_buffer[mate_proc], recv_counter[mate_proc], REFINED_INFO, mate_proc, refinedtag,
		    MPI_COMM_WORLD, &recv_request[mate_proc]);
		...
		for (int k = 0; k < recv_counter[mate_proc]; k++) {
			...
			if (!EmTemp->get_refined_flag()) {
				...
				if (my_gen == neighbor_gen) {
					EmTemp->change_neighbor(&recv_buffer[mate_proc][k].sons[0][0], which_neighbor,
					    neighbor_proc, 10);
					NodeTemp = (Node*) HT_Node_Ptr->lookup(
					    EmTemp->getNode() + (which_neighbor + 4) * KEYLENGTH);
					assert(NodeTemp);
					...
					if (NodeTemp->getinfo() == S_C_CON)
						...
						NodeTemp = (Node*) HT_Node_Ptr->lookup(
						    EmTemp->getNode() + (1 + which_neighbor) * KEYLENGTH);
						if (NodeTemp->getinfo() == S_C_CON)
							NodeTemp->putinfo(CORNER);
			...
			} else //this element was also refined
			{
				...
				for (int l = 0; l < 2; l++) {
					Son = (Element*) HT_Elem_Ptr->lookup(
					    EmTemp->getson() + KEYLENGTH * SIDE_SONS[which_neighbor][l]);
					Son->change_neighbor(&recv_buffer[mate_proc][k].sons[l][0], which_neighbor, neighbor_proc,
					    11);
