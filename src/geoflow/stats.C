...
void calc_stats(HashTable* El_Table, HashTable* NodeTable, int myid, MatProps* matprops,
	...
	if (myid == 0) {
		...
		statprops->xvar = tempout[11] * (matprops->LENGTH_SCALE) * (matprops->LENGTH_SCALE)
		    / tempout[10] - (statprops->xcen) * (statprops->xcen);
		statprops->yvar = tempout[12] * (matprops->LENGTH_SCALE) * (matprops->LENGTH_SCALE)
		    / tempout[10] - (statprops->ycen) * (statprops->ycen);
...
//print    runid  xC yC   rC  area maxh    wall time       cpu time
	fprintf(fp, "%6d   %E %E   %E   %E   %E   %E   %d:%02d:%02d   %d:%02d:%02d\n", statprops->runid,
	    statprops->xcen, statprops->ycen, statprops->rmean, statprops->area, statprops->hmax,
	    statprops->timereached, wallhours, wallminutes, wallseconds, cpuhours, cpuminutes,
	    cpuseconds);
	fflush(fp);
	fclose(fp);
