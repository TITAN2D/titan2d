...
void mark_flux_region(HashTable* ElemTable, HashTable* NodeTable, MatProps *matprops,
	...
	for (inode = 0; inode < 9; inode++) {
		...
		for (int isrc = 0; isrc < no_of_sources; isrc++) {
			...
			if ((fluxprops->start_time[isrc] <= curr_time) && (fluxprops->end_time[isrc] >= curr_time)) {
				...
				dswap = (major * fluxprops->cosrot[isrc] + minor * fluxprops->sinrot[isrc])
				    / fluxprops->majorrad[isrc];
				minor = (-major * fluxprops->sinrot[isrc] + minor * fluxprops->cosrot[isrc])
				...
				double tempflux = (fluxprops->influx[isrc])
				    * (1.0
				        - (curr_time - fluxprops->start_time[isrc])
