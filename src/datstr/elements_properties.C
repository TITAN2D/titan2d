...
void ElementsProperties::calc_flux(ti_ndx_t ndx,FluxProps *fluxprops, TimeProps *timeprops)
{
    ...
    for(inode = 0; inode < 9; inode++)
    {
        ...
        if(temp_coef2 > 0.0)
        {
            Influx_[0][ndx]+=temp_coef2;

            //if(sum_flux_xmom_ymom[0]>0.0) {
            Influx_[1][ndx] += sum_flux_xmom_ymom[1] * temp_coef2 / sum_flux_xmom_ymom[0];
            Influx_[2][ndx] += sum_flux_xmom_ymom[2] * temp_coef2 / sum_flux_xmom_ymom[0];
            //}
        }
