...
{ 
  ...
  prev_state_vars[0]=prev_state_vars[1]=prev_state_vars[2]=
    d_state_vars[0]=d_state_vars[1]=d_state_vars[2]=
    d_state_vars[3]=d_state_vars[4]=d_state_vars[5]=0.0;
...
double Element::calc_elem_edge_wetness_factor(int ineigh, double dt) {
  ...
  if(wetnessfactor>0.0) return 1.0;  //for simpleness ... may change later but will need to change most of this function's logic
...
void Element::xdirflux(MatProps* matprops_ptr, double dz, double wetnessfactor,
		       double hfv[3][3], double hrfv[3][3]){
...
void Element::ydirflux(MatProps* matprops_ptr, double dz, double wetnessfactor,
  ...
  //  printf("ydirflux\nspeed=%12.6g   a=%12.6g\nhfv=\n%12.6g   %12.6g   %12.6g\n%12.6g   %12.6g   %12.6g\n%12.6g   %12.6g   %12.6g\n",speed,a,hfv[0][0],hfv[1][0],hfv[2][0],hfv[0][1],hfv[1][1],hfv[2][1],hfv[0][2],hfv[1][2],hfv[2][2]);

  return;
}

//note z is not "z" but either x or y
 void Element::zdirflux(HashTable* El_Table, HashTable* NodeTable, MatProps* matprops_ptr, int order_flag, int dir, double hfv[3][3], double hrfv[3][3], Element *EmNeigh, double dt){
  double dz=0.0;
...
void riemannflux(double hfvl[3][NUM_STATE_VARS], double hfvr[3][NUM_STATE_VARS], double flux[NUM_STATE_VARS]){
...
void Element::correct(HashTable* NodeTable, HashTable* El_Table,
...
#ifdef SHORTSPEED
  if(state_vars[0]>GEOFLOW_TINY) {
    double  Vmag=pow(state_vars[1]*state_vars[1]+
		     state_vars[2]*state_vars[2],0.5);
    if(!(Vmag>0.0)) return;
    ...
    double doubleswap_h  =(state_vars[1]*d_state_vars[0]+
			   state_vars[2]*d_state_vars[3])*invnormhv;
    double doubleswap_hvx=(state_vars[1]*d_state_vars[1]+
			   state_vars[2]*d_state_vars[4])*invnormhv;
    double doubleswap_hvy=(state_vars[1]*d_state_vars[2]+
			   state_vars[2]*d_state_vars[5])*invnormhv;
...
double* Element::eval_velocity(double xoffset, double yoffset, double VxVy[2]) {
  if(!(shortspeed>=0.0))
    printf("shortspeed=%g\n",shortspeed);

  assert(shortspeed>=0.0);

  //assert((fabs(nondimx)<=0.5)&&(fabs(nondimy)<=0.5));
    
  //double distfromcenter[2];
  //distfromcenter[0]=nondimx*dx[0];
  //distfromcenter[1]=nondimx*dx[1];
...
#ifdef SHORTSPEED
  double doubleswap=
    (temp_state_vars[1])*(temp_state_vars[1])+
  ...
  if((temp_state_vars[0]<GEOFLOW_SHORT) &&
     (doubleswap>shortspeed*shortspeed*temp_state_vars[0]*temp_state_vars[0])){
    doubleswap=sqrt(doubleswap);
    VxVy[0]=shortspeed*temp_state_vars[1]/doubleswap;
    VxVy[1]=shortspeed*temp_state_vars[2]/doubleswap;
  }
  else
#endif
    {
      VxVy[0]=temp_state_vars[1]/temp_state_vars[0];
      VxVy[1]=temp_state_vars[2]/temp_state_vars[0];
    }    

  assert((VxVy[0]*VxVy[0]*VxVy[1]*VxVy[1])>=0.0);
  ...
  if(state_vars[0]<=GEOFLOW_TINY) stopcrit=HUGE_VAL;
	...
	(dirx*effect_kactxy[0]*d_state_vars[0]+
	 diry*effect_kactxy[1]*d_state_vars[NUM_STATE_VARS]);
      ...
      stopcrit=(slopetemp+effect_tanbedfrict)/Vtemp*NUM_FREEFALLS_2_STOP*sqrt(2.0*9.8/matprops_ptr->GRAVITY_SCALE*state_vars[0]/(1+bedslope*bedslope));     
    }
    else{
...
int Element::if_source_boundary(HashTable *ElemTable){
  ...
  if(!(Influx[0]>=0.0)){ printf("if_source_boundary() Influx[0]=%g\n",Influx[0]); fflush(stdout);}
...
int Element::if_first_buffer_boundary(HashTable *ElemTable, double contour_height){
  ...
  else{/* if((state_vars[0]>=contour_height)||
	  (Influx[0]>0)){*/
    for(ineigh=0;ineigh<8;ineigh++)
      if(neigh_proc[ineigh]>=0){ //don't check outside map boundary or duplicate neighbor
	ElemNeigh=(Element*) ElemTable->lookup(neighbor[ineigh]);
...
int Element::if_next_buffer_boundary(HashTable *ElemTable, HashTable *NodeTable, double contour_height){
  ...
  if((adapted!=BUFFER)&& //this element is not in the buffer
	...
	if((abs(ElemNeigh->get_adapted_flag())==BUFFER)&&
	   (state_vars[0]<=*(ElemNeigh->get_state_vars()+0)))
	  //if(abs(ElemNeigh->get_adapted_flag())==BUFFER)
