...
Element::Element(unsigned nodekeys[][KEYLENGTH], unsigned neigh[][KEYLENGTH], int n_pro[], BC* b,
	...
	prev_state_vars[0] = prev_state_vars[1] = prev_state_vars[2] = d_state_vars[0] = d_state_vars[1] =
	    d_state_vars[2] = d_state_vars[3] = d_state_vars[4] = d_state_vars[5] = 0.0;
...
void Element::xdirflux(MatProps* matprops_ptr, double dz, double wetnessfactor, double hfv[3][3],
    double hrfv[3][3]) {
...
void Element::ydirflux(MatProps* matprops_ptr, double dz, double wetnessfactor, double hfv[3][3],
    double hrfv[3][3]) {
...
void Element::zdirflux(HashTable* El_Table, HashTable* NodeTable, MatProps* matprops_ptr,
    int order_flag, int dir, double hfv[3][3], double hrfv[3][3], Element *EmNeigh, double dt) {
	double dz = 0.0;
...
void Element::correct(HashTable* NodeTable, HashTable* El_Table, double dt, MatProps* matprops_ptr,
	...
	correct_(state_vars, prev_state_vars, fluxxp, fluxyp, fluxxm, fluxym, &tiny, &dtdx, &dtdy, &dt,
	    ...
	    &(matprops_ptr->frict_tiny), forceint, forcebed, &do_erosion, eroded, VxVy, //eval_velocity(0.0,0.0,VxVy),
	    &IF_STOPPED, Influx);
	...
	if (state_vars[0] <= GEOFLOW_TINY)
		...
		if (Vtemp > 0) {
			...
			stopcrit = (slopetemp + effect_tanbedfrict) / Vtemp * NUM_FREEFALLS_2_STOP
			    * sqrt(
			        2.0 * 9.8 / matprops_ptr->GRAVITY_SCALE * state_vars[0] / (1 + bedslope * bedslope));
		} else {
