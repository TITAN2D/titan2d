...
void TitanPreproc::createfunky(double limits[4], int *node_count, NodePreproc **node, int *element_count,
                               ElementPreproc **element, int *force_count, int *constraint_count,
                               BoundaryPreproc **boundary, int *material_count, char ***materialnames, double **lambda,
                               double **mu)
{
    ...
    if(!Initialize_GIS_data(topomain.c_str(), toposub.c_str(), topomapset.c_str(), topomap.c_str(), gis_format))
    {
        ...
        *boundary = (BoundaryPreproc *) calloc(2 * (nx + ny), sizeof(BoundaryPreproc));
        ...
        for(j = 0; j < 2 * ny + 1; j++)
            for(i = 0; i < 2 * nx + 1; i++)
                if(!(i % 2) || !(j % 2))
                { //only output side and corner nodes, not bubble
                    xy[inode][0] = x[i];
                    xy[inode][1] = y[j];
                    ...
                    //if on a boundary
                    if((((j == 0) || (j == 2 * ny)) && (i % 2)) || (((i == 0) || (i == 2 * nx)) && (j % 2)))
                    {
