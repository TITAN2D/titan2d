...
void ElementPreproc::write_element_data(ofstream* out)
{
    ...
    *out << '\n';
    ...
    {
        if(neighbor[i] != 0)
            *out << neighbor[i]->myprocess << ' ' << neighbor[i]->element_nodes[8]->key[0] << ' '
                 << neighbor[i]->element_nodes[8]->key[1] << ' ';
        else
            *out << -1 << ' ';
    }
    /*  *out<<elementid<<'\n';
    ...
    *out << '\n';
    ...
    {
        if(boundary[i][0] != 0)
            *out << boundary[i][0]->type << ' ' << boundary[i][0]->x_value << ' ' << boundary[i][0]->y_value << ' ';
        else
            *out << -1 << ' ';
    }
    *out << '\n';
    ...
    {
        if(boundary[i][1] != 0)
            *out << boundary[i][1]->type << ' ' << boundary[i][1]->x_value << ' ' << boundary[i][1]->y_value << ' ';
        else
            *out << -1 << ' ';
    }
