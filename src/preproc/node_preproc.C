 ...
 *
 *******************************************************************
 * $Id: node.C 224 2011-12-04 20:49:23Z dkumar $ 
 */
...

{
    if(node_coord[0] > max[0])
        max[0] = node_coord[0];
    if(node_coord[1] > max[1])
        max[1] = node_coord[1];
    if(node_coord[0] < min[0])
        min[0] = node_coord[0];
    if(node_coord[1] < min[1])
        min[1] = node_coord[1];
    
}

void NodePreproc::determine_the_key(unsigned nkey, double* max, double *min, unsigned ma[], unsigned mi[])
{
    ...
    if(key[0] > ma[0] || (key[0] == ma[0] && key[1] > ma[1]))
    {
        ma[0] = key[0];
        ma[1] = key[1];
    }
    if(key[0] < mi[0] || (key[0] == mi[0] && key[1] < mi[1]))
    {
        mi[0] = key[0];
        mi[1] = key[1];
    }
...
{
    if(!written)
        *out << key[0] << ' ' << key[1] << ' ' << setprecision(20) << node_coord[0] << ' ' << setprecision(20)
             << node_coord[1] << '\n';
    written = 1;
}
