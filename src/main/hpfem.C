...
int main(int argc, char *argv[]) 
{
  ...
  */
  ...
  while(!(timeprops.ifend(0)) && !ifstop)
	...
	unrefine(BT_Elem_Ptr, BT_Node_Ptr, UNREFINE_TARGET, myid, 
                 numprocs, &timeprops, &matprops);
  ...
  if (numprocs > 1 )
  {
    ...
    MPI_Reduce(*(outline.max_kinergy), *(outline2.max_kinergy), NxNyout, 
               MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(*(outline.cum_kinergy), *(outline2.cum_kinergy), NxNyout, 
