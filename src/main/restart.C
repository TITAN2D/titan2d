...
int loadrun(int myid, int numprocs, HashTable** NodeTable, ElementsHashTable** ElemTable,
	...
	if (strncmp(header + Itemp, "one-phase:20060801", temp4.i)) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" it version is \"%s\" instead of \"one-phase:20051027\" aborting\n",
	...
	if (temp4.i != myid) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" was written by processor %d instead of %d, aborting\n",
	...
	if (temp4.i != numprocs) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" originally run on %d processors, current job is using %d processors, aborting\n",
	...
	if (temp4.i != EQUATIONS) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" which originally had %d equations, this version of titan uses %d equations, aborting\n",
	...
	*order_flag_ptr = temp4.i;
	...
	timeprops_ptr->ioutput = floor(
	    timeprops_ptr->time * timeprops_ptr->TIME_SCALE / timeprops_ptr->timeoutput);
	timeprops_ptr->ndnextoutput = ((timeprops_ptr->ioutput + 1) * timeprops_ptr->timeoutput)
	    / timeprops_ptr->TIME_SCALE;

	timeprops_ptr->isave = floor(
	    timeprops_ptr->time * timeprops_ptr->TIME_SCALE / timeprops_ptr->timesave);
	timeprops_ptr->ndnextsave = ((timeprops_ptr->isave + 1) * timeprops_ptr->timesave)
	    / timeprops_ptr->TIME_SCALE;
	...
	if (strncmp(header + Itemp, mapnames_ptr->gis_main, temp4.i)) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" old GIS main directory=\"%s\" new GIS main directory=\"%s\" proceeding anyway\n",
	...
	if (strncmp(header + Itemp, mapnames_ptr->gis_sub, temp4.i)) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" old GIS sub directory=\"%s\" new GIS sub directory=\"%s\" proceeding anyway\n",
	...
	if (strncmp(header + Itemp, mapnames_ptr->gis_mapset, temp4.i)) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" old GIS mapset =\"%s\" new GIS mapset =\"%s\" proceeding anyway\n",
	...
	if (strncmp(header + Itemp, mapnames_ptr->gis_map, temp4.i)) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" old GIS map =\"%s\" new GIS map =\"%s\" aborting\n",
	...
	if (temp4.i != mapnames_ptr->extramaps) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" old had extramaps =%d new has extramaps =%d, aborting\n",
	...
	if (temp4.i != matprops_ptr->material_count) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" old had %d materials new had=%d materials, aborting\n",
	...
	for (imat = 1; imat <= matprops_ptr->material_count; imat++) {
		...
		if (temp8.d != matprops_ptr->bedfrict[imat]) {
			printf(
			    "in loadrun() processor %d opened file \"%s\" for material %d new bed friction=%g [deg] old bed friction =%g [deg]\n",
	...
	if (temp8.d != matprops_ptr->intfrict) {
		printf(
		    "in loadrun() processor %d opened file \"%s\" new internal friction=%g [deg] old internal friction =%g [deg]\n",
	...
	if (myid == 0)
		outline_ptr->reload(matprops_ptr, statprops_ptr);
	...
	*NodeTable = new HashTable(doublekeyrange, NODE_TABLE_SIZE, 2017, Xrange, Yrange, 1);
	...
	if (Itemp != numchar) {
		printf(
		    "DANGER!!! load_run() uses %d instead of %d bytes, load_run not compattible with save_run that wrote the file %s on processor %d\n",
	...
	*ElemTable = new ElementsHashTable(doublekeyrange, ELEM_TABLE_SIZE, 503, Xrange, Yrange, 1,*NodeTable);
	...
	/*
	 ...
	 if(myid==0) {
	 ElemTemp=(Element*) (*ElemTable)->lookup(tempkey);
	 
	 printf("prev_state_vars=%g %g %g\n",
	 *(ElemTemp->get_prev_state_vars()+0),
	 *(ElemTemp->get_prev_state_vars()+1),
	 *(ElemTemp->get_prev_state_vars()+2));
	 }
	 */
...
void saverun(HashTable** NodeTable, int myid, int numprocs, ElementsHashTable** ElemTable,
	...
	for (itemp = 0; itemp < temp4.i; itemp++)
		header[Itemp++] = versionstring[itemp];
#ifdef DEBUGSAVEHEADER
	...
	if (Itemp >= NUM_CHAR_IN_SAVE_HEADER) {
		printf(
		    "In restart.C while saving, ran out of room in header;\nItemp=%d>=NUM_CHAR_IN_SAVE_HEADER=%d\nABORTING!!!!\n",
