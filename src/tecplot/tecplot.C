...
void
testkey (HashTable * El_Table, Element * EmTemp)
{
  unsigned key[2];
 ...
 */
void
get_elem_orient (Element * EmTemp, int *xm, int *xp, int *ym, int *yp)
{
...
*/
int
get_ll_polygon (HashTable * El_Table, HashTable * NodeTable,
                int myid, Element * EmArray[4])
{
  ...
  EmArray[1] =
    (Element *) El_Table->lookup (EmArray[0]->get_neighbors () +
                                  (xm + 4) * KEYLENGTH);
  EmArray[2] =
    (Element *) El_Table->lookup (EmArray[0]->get_neighbors () +
                                  ym * KEYLENGTH);
  ...
  if ((neigh_proc[xm + 4] == myid) ||
      ((neigh_proc[xm + 4] == -2) && (neigh_proc[xm] == myid)))
    {
      ...
      EmArray[3] =
        (Element *) El_Table->lookup (EmArray[1]->get_neighbors () +
                                      (zmym + 4) * KEYLENGTH);
    ...
    {                           //neigh_proc[ym]==myid
      ...
      EmArray[3] =
        (Element *) El_Table->lookup (EmArray[2]->get_neighbors () +
                                      zmxm * KEYLENGTH);
    }
...
int
get_ur_tri (HashTable * El_Table, HashTable * NodeTable,
            int myid, Element * EmArray[4])
{
  int xm, xp, ym, yp;           //this  element's  left right down up 
...
void
viz_output (HashTable * El_Table, HashTable * NodeTable,
              ...
              if (state_vars[0] < GEOFLOW_TINY)
                {
                  ...
                  fprintf (fp, "%u %u %f %f %f %f %f %f %f %f %d %f \n",
                           ...
                           *(EmTemp->pass_key () + 1),
                           ...
                           (*(EmTemp->get_coord () + 1)) *
                           (matprops)->LENGTH_SCALE,
                           (NdTemp->get_elevation ()) *
                           (matprops)->LENGTH_SCALE, *(EmTemp->get_zeta ()),
                           *(EmTemp->get_zeta () + 1), zero, zero, zero, myid,
                           zero);
                }
              else
                {
                  double VxVy[2];
                  EmTemp->eval_velocity (0.0, 0.0, VxVy);
                  fprintf (fp, "%u %u %f %f %f %f %f %f %f %f %d %f \n", *(EmTemp->pass_key ()), *(EmTemp->pass_key () + 1), (*(EmTemp->get_coord ())) * (matprops)->LENGTH_SCALE, (*(EmTemp->get_coord () + 1)) * (matprops)->LENGTH_SCALE, (NdTemp->get_elevation ()) * (matprops)->LENGTH_SCALE, *(EmTemp->get_zeta ()), *(EmTemp->get_zeta () + 1), state_vars[0] * (matprops)->HEIGHT_SCALE, velocity_scale * VxVy[0],     //*state_vars[1]/state_vars[0],
          ...
          if (EmTemp->get_adapted_flag () > 0)
            {
              ...
              for (int j = 0; j < 4; j++)
                {
                  NodeTemp =
                    (Node *) NodeTable->lookup (nodes + j * KEYLENGTH);
                  assert (NodeTemp);
                  ...
                  if (1)
                    {           //NodeTemp->getinfo() != S_C_CON) {
                      fprintf (fp,
                               "%e %e %e %d %e %e %e %u %u %d %d %e %e %e %e %e %d %d\n",
                               (*(NodeTemp->get_coord ())) *
                               (matprops)->LENGTH_SCALE,
                               (*(NodeTemp->get_coord () + 1)) *
                               (matprops)->LENGTH_SCALE,
                               NodeTemp->get_elevation () *
                               (matprops->LENGTH_SCALE), myid,
                               state_vars[0] * (matprops)->HEIGHT_SCALE,
                               state_vars[1] * momentum_scale,
                               state_vars[2] * momentum_scale,
                               *(EmTemp->pass_key ()),
                               *(EmTemp->pass_key () + 1), EmTemp->get_gen (),
                               EmTemp->get_which_son (),
                               EmTemp->get_elevation () *
                               (matprops->LENGTH_SCALE),
                               *(EmTemp->get_zeta ()),
                               *(EmTemp->get_zeta () + 1),
                               *(EmTemp->get_curvature ()) /
                               (matprops->LENGTH_SCALE),
                               *(EmTemp->get_curvature () +
                                 1) / (matprops->LENGTH_SCALE),
                               *(EmTemp->get_elm_loc ()),
                    ...
                    {           // S_C_CON will have a discontinuity in the elevation so fix that by interpolation
                      ...
                      Element *EmTemp2 =
                        (Element *) El_Table->
                        lookup ((EmTemp->get_neighbors () +
                                 KEYLENGTH * neighside));
                      assert (EmTemp2);

                      NodeTemp2 =
                        (Node *) NodeTable->lookup (EmTemp2->getNode () +
                                                    j * KEYLENGTH);
                      elev += .5 * NodeTemp2->get_elevation ();
                      fprintf (fp,
                               "%e %e %e %d %e %e %e %u %u %d %d %e %e %e %e %e %d %d\n",
                               ...
                               (*(NodeTemp->get_coord () + 1)) *
                               (matprops)->LENGTH_SCALE,
                               elev * (matprops->LENGTH_SCALE), myid,
                               state_vars[0] * (matprops)->HEIGHT_SCALE,
                               state_vars[1] * momentum_scale,
                               state_vars[2] * momentum_scale,
                               *(EmTemp->pass_key ()),
...
*************************************/
void
vizplotter (HashTable * El_Table, HashTable * NodeTable, MatProps * matprops,
            TimeProps * timeprops)
{
  int myid, i;
  int numprocs;
