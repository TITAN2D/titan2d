...
CGIS_TriGrid::CGIS_TriGrid(char *GIS_Dbase, char *GIS_Maplocation, char *GIS_Mapset,
    char *GIS_Rasterimage) {

	int err = Initialize_GIS_data(GIS_Dbase, GIS_Maplocation, GIS_Mapset, GIS_Rasterimage);

	if (err) {
		cout << "\n\nERROR: Unable to initialize GIS for " << GIS_Dbase << " : " << GIS_Maplocation
		    << " : " << GIS_Mapset << " : " << GIS_Rasterimage << endl;
		exit(1);
	}
	...
	for (int i = 0; i < _num_of_rows; ++i) {
		for (int j = 0; j < _num_of_cols; ++j) {
			...
			if (Get_elevation(_res, _xmin + j * _res, _ymin + i * _res,
			    &_elevations[i * _num_of_cols + j])) {
