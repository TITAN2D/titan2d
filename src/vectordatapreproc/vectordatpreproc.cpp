...
#include "vectordatpreproc.h"

int vectordatpreproc(const char * topomain, const char * toposub, const char * topomapset, const char * topomap, const char * topovector) {
    /*if (argc < 6) {
        cout
            << "ERROR: Usage ./vectordatapreprocessor <GIS Dbase> <GIS Maplocation> <GIS Mapset> <GIS Rasterimage> <GIS Vectorfile>\n";
        return EXIT_FAILURE;
    }*/

    //Reads in the DEM elevations and stores them in a 2D array
    //CGIS_TriGrid* _triGrid = new CGIS_TriGrid(topomain, toposub, topomapset, topomap);
    ...
    int err = Initialize_GIS_data(topomain, toposub, topomapset, topomap);
    ...
    CGIS_VectorData* _vecData = new CGIS_VectorData(res, xmin, xmax, ymin, ymax, rows, cols, topomain,
        toposub, topomapset, topovector);
