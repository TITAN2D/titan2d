...
int main() {
	...
	fprintf(fpout,
	    "#Nsamp prob_reached    Vave_mean Vave_std_dev    Vave_skew    hmax_mean hmax_std_dev    hmax_skew    xcen_mean xcen_std_dev    xcen_skew    ycen_mean ycen_std_dev    ycen_skew xspread_mean xspr_std_dev    xspr_skew yspread_mean yspr_std_dev    yspr_skew\n");

	while (fpin != NULL) {
		//printf("%s\n",filename);

		do {
			fscanf(fpin, "%2d%6d%lf%lf%lf%lf%lf%lf%lf\n", &refnum, &isample, &timereached, &Vave, &hmax,
			    &xcen, &ycen, &xspread, &yspread);
		...
		for (i = 0; i < 6; i++) {
			for (j = 0; j < 3; j++)
				stats[j] = statmoments[i][j] / Nsample;
