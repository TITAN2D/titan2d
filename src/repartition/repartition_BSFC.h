...
int BSFC_find_imbalance(float* work_percent_array, float cumulative_work, float total_work, int which_proc,
                        int numprocs);

void BSFC_single_wgt_calc_partition(int wgt_dim, float work_allocated, float* total_weight_array, int* bin_proc_array,
                                    float* binned_weight_array, float* work_percent_array, float* actual_work_allocated,
                                    int number_of_bins, int* number_of_cuts, int current_proc, int level_flag, int*);

int BSFC_get_array_location(int number_of_bins, int number_of_bits, int prev_used_bits, BSFC_VERTEX_PTR sfc_vert_ptr);

void BSFC_get_normed_coords(double min_bounding_box[], double max_bounding_box[], double normed_coords[], int num_dims,
                            double my_coords[]);

void BSFC_create_info(double min_bounding_box[], double max_bounding_box[], int num_dims, int num_local_objects,
                      int wgt_dim, BSFC_VERTEX_PTR sfc_vert_ptr, double* coords);
...
int BSFC_create_compare_key(unsigned sfc_key[], unsigned compare_key[], unsigned AND_operator_array[],
                            int prev_used_bits);
