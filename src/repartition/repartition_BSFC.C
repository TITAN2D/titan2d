...
void BSFC_update_element_proc(int myid, int numprocs, HashTable* HT_Elem_Ptr,
    HashTable* HT_Node_Ptr, BSFC_VERTEX_PTR sfc_vert_ptr);
...
void repartition(HashTable* HT_Elem_Ptr, HashTable* HT_Node_Ptr, int time_step) {
	...
	if (balanced_flag != BSFC_BALANCED) {
		...
		BSFC_create_refinement_info(&number_of_cuts, global_actual_work_allocated, total_weight,
		    work_percent_array, verts_in_cut_info, &work_prev_allocated, myid, numprocs);
			...
			if (i != myid) {
				...
				if (verts_in_cut_info.recv_procs_ptr[i] != 0) {
					j = MPI_Isend(&(verts_in_cut_info.recv_sfc_vert[recv_count]),
					    verts_in_cut_info.recv_procs_ptr[i], LB_VERT_TYPE, i, tag, MPI_COMM_WORLD,
					    (recv_request + i));
				}
			}
...
void BSFC_create_refinement_info(int* number_of_cuts, float* global_actual_work_allocated,
    float total_weight, float* work_percent_array, unstructured_communication verts_in_cut_info,
...
int SequentialSend(int numprocs, int myid, HashTable* El_Table, HashTable* NodeTable,
	...
	double tempsenddouble[3] = { (double) MyFirstAndLastKey[0], (double) MyFirstAndLastKey[KEYLENGTH],
	...
	if (timeprops_ptr->iter == DEBUG_ITER) {
		...
		fprintf(fpdb2,
		    ...
		    myid, MyFirstAndLastKey[0], MyFirstAndLastKey[1], MyFirstAndLastKey[2],
		    MyFirstAndLastKey[3], doublekeyrange1, tempsenddouble[0], tempsenddouble[1],
		    tempsenddouble[2]);
		fflush(stdout);
	...
	if (timeprops_ptr->iter == DEBUG_ITER) {
		...
		fprintf(fpdb2, "myid=%d repart2 num_send1={%d,%d} num_send2={%d,%d}\n", myid, num_send1[0],
		    num_send1[1], num_send2[0], num_send2[1]);
		fclose(fpdb2);
	}
	...
	if (myid > 0) {
		...
		if (((num_send1[0] < MIN_NUM_2_SEND) && (num_send2[0] == 0))
		    || (((num_recv1[0] >= MIN_NUM_2_SEND) || (num_recv2[0] > 0))
	...
	if (myid < numprocs - 1) {
		...
		if (((num_send1[1] < MIN_NUM_2_SEND) && (num_send2[1] == 0))
		    || (((num_recv1[1] >= MIN_NUM_2_SEND) || (num_recv2[1] > 0))
	...
	if (If2WaySend[0]) {
		...
		ierr = MPI_Isend((void *) DoubleKeyMy2WaySend0, NumMy2WayElem, MPI_DOUBLE, myid - 1,
		    send_tag1 + myid, MPI_COMM_WORLD, &(Request2WaySend[0]));
	}
	...
	if (If2WaySend[1]) {
		...
		ierr = MPI_Isend((void *) DoubleKeyMy2WaySend1, NumMy2WayElem, MPI_DOUBLE, myid + 1,
		    send_tag1 + myid, MPI_COMM_WORLD, &(Request2WaySend[1]));
	}
				...
				if (IfSentRecvd) {
					...
					if (timeprops_ptr->iter == DEBUG_ITER) {
						...
						fprintf(fpdebug,
						    ...
						    num_send[1], DoubleKeyMy2WaySend1[(num_elem - 1) / 2 - 1],
						    (num_neigh_elem[1] - 1) / 2, num_recv1[1], num_recv2[1], num_recv[1],
						    DoubleKeyYour2WaySend1[0]);
	...
	if (timeprops_ptr->iter == DEBUG_ITER) {
		...
		fprintf(fpdb2,
		    ...
		    myid, num_elem, num_send[0], num_send[1], num_send1[0], num_send1[1], num_send2[0],
		    num_send2[1], num_recv[0], num_recv[1], num_recv1[0], num_recv1[1], num_recv2[0],
		    num_recv2[1]);
		fclose(fpdb2);
		...
		if (timeprops_ptr->iter == DEBUG_ITER) {
			...
			if (num_send[0] == 0)
				fprintf(fpdb2, "myid=%d yadaA isend[0]=%d num_send[0]=%d num_elem=%d ielem=%d\n", myid,
				    isend[0], num_send[0], num_elem, ielem);
			else
				fprintf(fpdb2,
				    "myid=%d yadaA isend[0]=%d num_send[0]=%d num_elem=%d ielem=%d {%10u,%10u}\n", myid,
	...
	if (num_recv[0] > 0) {
		...
		ierr = MPI_Irecv((void *) recv_array0, num_recv[0], ELEMTYPE, myid - 1, send_tag2 + myid - 1,
		...
		if (timeprops_ptr->iter == DEBUG_ITER) {
			...
			if (num_send[1] == 0)
				fprintf(fpdb2, "myid=%d yadaB isend[1]=%d num_send[1]=%d num_elem=%d ielem=%d\n", myid,
				    isend[1], num_send[1], num_elem, ielem);
			else
				fprintf(fpdb2,
				    "myid=%d yadaB isend[1]=%d num_send[1]=%d num_elem=%d ielem=%d {%10u,%10u}\n", myid,
	...
	if (num_recv[1]) {
		...
		ierr = MPI_Irecv((void *) recv_array1, num_recv[1], ELEMTYPE, myid + 1, send_tag2 + myid + 1,
		    MPI_COMM_WORLD, requestrecv + 1);
	...
	if (timeprops_ptr->iter == DEBUG_ITER) {
		...
		fprintf(fpdb2, "myid=%d B) MyFirstAndLastKey={{%10u,%10u},{%10u,%10u}}\n", myid,
		    MyFirstAndLastKey[0], MyFirstAndLastKey[1], MyFirstAndLastKey[2], MyFirstAndLastKey[3]);
		fflush(stdout);
	...
	if (timeprops_ptr->iter == DEBUG_ITER) {
		...
		for (ielem = 0; ielem <= isend[0]; ielem++)
			fprintf(fpdb2, "    deleting %dth element: key={%u,%u}\n", ielem,
			    *(ElemArray[ielem]->pass_key() + 0), *(ElemArray[ielem]->pass_key() + 1));
			...
			for (inode = 0; inode < 8; inode++) {
				...
				if (NdTemp == NULL) {
					...
					fprintf(fpdb2, "myid=%d iter=%d iseqsend=%d node inode=%d is missing\n", myid,
					    timeprops_ptr->iter, iseqsend, inode);
	...
	for (ikey = 1; ikey < KEYLENGTH; ikey++) {
		MyFirstAndLastDoubleKey[0] = MyFirstAndLastDoubleKey[0] * doublekeyrange1
		    + MyFirstAndLastKey[ikey];
		MyFirstAndLastDoubleKey[1] = MyFirstAndLastDoubleKey[1] * doublekeyrange1
	...
	MPI_Allgather(MyFirstAndLastDoubleKey, 2, MPI_DOUBLE, *AllFirstAndLastDoubleKeys, 2, MPI_DOUBLE,
	    MPI_COMM_WORLD);
	...
	for (iproc = 0; iproc < numprocs; iproc++) {
		...
		for (jproc = iproc + 1; jproc < numprocs; jproc++)
			if ((AllFirstAndLastDoubleKeys[iproc][0] > AllFirstAndLastDoubleKeys[jproc][0])
			    || (AllFirstAndLastDoubleKeys[iproc][1] > AllFirstAndLastDoubleKeys[jproc][1])) {
				ifrepeat = 1;
				break;
			}
		if (ifrepeat)
...
void NonSequentialSendAndUpdateNeigh(int numprocs, int myid, HashTable* El_Table,
	...
	if (timeprops_ptr->iter == DEBUG_ITER) {
		...
		for (iproc = 0; iproc <= numprocs; iproc++)
			fprintf(fpdb2, "   NewProcDoubleKeyBoundaries[%3d]=%20.14g\n", iproc,
			    NewProcDoubleKeyBoundaries[iproc]);
	...
	for (ielem = 0; ielem < NotMyElem.get_num_elem(); ielem++) {
		...
		for (iproc = 0; iproc < numprocs; iproc++)
			if ((NewProcDoubleKeyBoundaries[iproc] < doublekey)
				...
				Pack_element(NotMyElem.get(ielem), SecondSendArray[iproc] + IfSecondSendDone[iproc],
				    NodeTable, iproc);
	...
	for (iproc = 0; iproc < numprocs; iproc++) {
		...
		if (timeprops_ptr->iter == DEBUG_ITER) {
			fpdb2 = fopen(fname2, "a");
			fprintf(fpdb2,
			    "myid=%d iter=%d iproc=%d IfSecondSendDone=%d NumToSecondSend=%d NumNotMyElem=%d\n", myid,
		...
		if (NumToSecondSend[iproc] > 0) {
			...
			if (timeprops_ptr->iter == DEBUG_ITER) {
				fpdb2 = fopen(fname2, "a");
				fprintf(fpdb2,
				    "myid=%d iter=%d did a non blocking nonsequential send to proc %d  #elem sent=%d\n",
