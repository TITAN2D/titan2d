...
void BSFC_create_refinement_info(int* number_of_cuts, float* global_actual_work_allocated, float total_weight,
                                 float* work_percent_array, unstructured_communication verts_in_cuts_info, float**,
                                 int myid, int numprocs);

void BSFC_create_bins(int num_local_objects, BSFC_VERTEX_PTR sfc_vert_ptr, int* amount_of_bits_used,
                      int size_of_unsigned, float* global_actual_work_allocated, float *work_percent_array,
...
void repartition(ElementsHashTable* HT_Elem_Ptr, NodeHashTable* HT_Node_Ptr, int time_step)
{
    ...
    BSFC_create_bins(num_local_objects, sfc_vert_ptr, &amount_of_bits_used, size_of_unsigned,
                     global_actual_work_allocated, work_percent_array, &total_weight, &balanced_flag,
                     &verts_in_cut_info, &number_of_cuts, BINS_PER_PROC, myid, numprocs);
    ...
    if(balanced_flag != BSFC_BALANCED)
    {
        ...
        BSFC_create_refinement_info(&number_of_cuts, global_actual_work_allocated, total_weight, work_percent_array,
                                    verts_in_cut_info, &work_prev_allocated, myid, numprocs);
        ...
        while (balanced_flag != BSFC_BALANCED && refinement_level_counter < MAX_REFINEMENT_LEVEL)
        {
            BSFC_refine_partition(&balanced_flag, &amount_of_bits_used, verts_in_cut_info.recv_count,
                                  verts_in_cut_info.recv_sfc_vert, work_percent_array, total_weight,
                                  global_actual_work_allocated, number_of_cuts, ll_bins_head, work_prev_allocated,
                                  SUBBINS_PER_BIN,
    ...
    {  // move back and free up the space...
        int recv_count = 0;
        ...
        for(i = 0; i < numprocs; i++)
        {
            if(i != myid)
            {
                ...
                {
                    j = MPI_Irecv((send_sfc_vert + proc_counter[i]), verts_in_cut_info.send_procs_ptr[i], LB_VERT_TYPE,
        ...
        for(i = 0; i < num_local_objects; i++)
            if(sfc_vert_ptr[i].cut_bin_flag == BSFC_CUT)
            {
                ...
                if(sfc_vert_ptr[i].destination_proc != myid)
                {
                    ...
                    sfc_vert_ptr[i].destination_proc = send_sfc_vert[proc_counter[sfc_vert_ptr[i].destination_proc]]
                            .destination_proc;
                    proc_counter[j] += 1;
                }
                else
...
void BSFC_create_refinement_info(int* number_of_cuts, float* global_actual_work_allocated, float total_weight,
                                 float* work_percent_array, unstructured_communication verts_in_cut_info,
                                 float** work_prev_allocated_ptr, int myid, int numprocs)
{
    float my_work, work2;
    int i = 0, j;
    ...
    *work_prev_allocated_ptr = (float*) malloc(sizeof(float) * numprocs);
...
void repartition2(ElementsHashTable* El_Table, NodeHashTable* NodeTable, TimeProps* timeprops_ptr)
{
    ...
    do
    {
        ifrepeat = SequentialSend(numprocs, myid, El_Table, NodeTable, timeprops_ptr, NewProcDoubleKeyBoundaries,
                                  counter);
    ...
    NonSequentialSendAndUpdateNeigh(numprocs, myid, El_Table, NodeTable, timeprops_ptr, NewProcDoubleKeyBoundaries);
...
/*void checkelemnode(HashTable *El_Table, HashTable *NodeTable, int myid, FILE *fpdebug, double loc)
{
                   ...
                   double *NewProcDoubleKeyBoundaries, int iseqsend)
{
        ...
        while (currentPtr)
        {
            ...
            if(EmTemp->adapted_flag() >= NOTRECADAPTED)
            {
                ...
                for(ikey = 1; ikey < KEYLENGTH; ikey++)
                    DoubleKeyArray[ielem] = DoubleKeyArray[ielem] * doublekeyrange1 + tmpkey[ikey];
    ...
    //store my first key
    SET_OLDKEY(((MyFirstAndLastKey+0 * KEYLENGTH)),ElemArray[0]->key());
    ...
    //amount of time  
    SET_OLDKEY(((MyFirstAndLastKey+1 * KEYLENGTH)),ElemArray[num_elem - 1]->key())
    ...
    { (double) MyFirstAndLastKey[0], (double) MyFirstAndLastKey[KEYLENGTH], LoadBalArray[num_elem - 1] };
    
    for(ikey = 1; ikey < KEYLENGTH; ikey++)
    {
        tempsenddouble[0] = tempsenddouble[0] * doublekeyrange1 + MyFirstAndLastKey[ikey];
        
        tempsenddouble[1] = tempsenddouble[1] * doublekeyrange1 + MyFirstAndLastKey[KEYLENGTH + ikey];
    }
    ...
    if(timeprops_ptr->iter == DEBUG_ITER)
    {
        ...
        fprintf(fpdb2,
                "myid=%d repart2 MyF&LKey={{%10u,%10u},{%10u,%10u}} doublekeyrange1=%20.14g tempsenddouble={%20.14g,%20.14g,%20.14g}\n",
                myid, MyFirstAndLastKey[0], MyFirstAndLastKey[1], MyFirstAndLastKey[2], MyFirstAndLastKey[3],
                doublekeyrange1, tempsenddouble[0], tempsenddouble[1], tempsenddouble[2]);
        fflush(stdout);
    ...
    MPI_Allgather(tempsenddouble, 3, MPI_DOUBLE, temprecvdouble, 3, MPI_DOUBLE, MPI_COMM_WORLD);
    ...
    if(timeprops_ptr->iter == DEBUG_ITER)
    {
        ...
        for(iproc = 0; iproc < numprocs; iproc++)
            fprintf(fpdb2, "    iproc=%d {%20.14g,%20.14g}\n", iproc, AllFirstAndLastDoubleKeys[iproc][0],
    ...
    int isend1[2] =
    { -1, num_elem }; //criteria 1 of first-send
    int isend2[2] =
    { -1, num_elem }; //criteria 2 of first-send
    ...
    if(myid < numprocs - 1)
        for(ielem = 0; ielem < num_elem; ielem++)
        {
            ...
            unsigned tmpkey[KEYLENGTH];
            SET_OLDKEY(tmpkey,(ElemArray[ielem]->key()));
            doublekey =  tmpkey[0];
            for(ikey = 1; ikey < KEYLENGTH; ikey++)
    ...
    //             after you on the space filling curve
    // **********************************************************************
    int NumMy2WayElem = (num_elem - 1) / 2, NumYour2WayElem[2] =
    { -1, -1 };
    int num_send[2] =
    { 0, 0 }; //final
    ...
    int num_send2[2] =
    { isend2[0] + 1, num_elem - isend2[1] }; //criteria 2
    ...
    { 0, 0 }; //criteria 2
    int num_recvmax[2] =
    { 0, 0 }; //max of criteria 1 and criteria 2
    ...
    int tempsendint[5] =
    { num_send1[0], num_send1[1], num_send2[0], num_send2[1], num_elem };
    int *temprecvint = CAllocI1(5 * numprocs);
    int num_neigh_elem[2] =
    { -1, -1 };
    int If2WaySend[2] =
    { 0, 0 };
    ...
    MPI_Allgather(tempsendint, 5, MPI_INT, temprecvint, 5, MPI_INT, MPI_COMM_WORLD);
    ...
    if(myid > 0)
    {
        ...
        if(((num_send1[0] < MIN_NUM_2_SEND) && (num_send2[0] == 0)) || (((num_recv1[0] >= MIN_NUM_2_SEND)
                || (num_recv2[0] > 0))
                                                                        && (num_recv1[0] >= num_recv2[0])))
        {
            num_send[0] = 0;
            if((num_recv1[0] >= MIN_NUM_2_SEND) || (num_recv2[0] > 0))
    ...
    if(myid < numprocs - 1)
    {
        ...
        if(((num_send1[1] < MIN_NUM_2_SEND) && (num_send2[1] == 0)) || (((num_recv1[1] >= MIN_NUM_2_SEND)
                || (num_recv2[1] > 0))
                                                                        && (num_recv1[1] >= num_recv2[1])))
        {
            num_send[1] = 0;
            if((num_recv1[1] >= MIN_NUM_2_SEND) || (num_recv2[1] > 0))
    ...
    if(timeprops_ptr->iter == DEBUG_ITER)
    {
        ...
        fprintf(fpdebug,
                "myid=%d SequentialSend() A If2WaySend[0]=%d\n  (num_elem      -1)/2=%6d num_send1[0]=%6d num_send2[0]=%6d num_send[0]=%6d\n  (num_neigh_elem-1)/2=%6d num_recv1[0]=%6d num_recv2[0]=%6d num_recv[0]=%6d\n",
                myid, If2WaySend[0], (num_elem - 1) / 2, num_send1[0], num_send2[0], num_send[0],
                (num_neigh_elem[0] - 1) / 2, num_recv1[0], num_recv2[0], num_recv[0]);
        fprintf(fpdebug,
                "myid=%d SequentialSend() B If2WaySend[1]=%d\n  (num_elem      -1)/2=%6d num_send1[1]=%6d num_send2[1]=%6d num_send[1]=%6d\n  (num_neigh_elem-1)/2=%6d num_recv1[1]=%6d num_recv2[1]=%6d num_recv[1]=%6d\n",
                myid, If2WaySend[1], (num_elem - 1) / 2, num_send1[1], num_send2[1], num_send[1],
                (num_neigh_elem[1] - 1) / 2, num_recv1[1], num_recv2[1], num_recv[1]);
    ...
    if(If2WaySend[0])
    {
        ...
        ierr = MPI_Irecv((void *) DoubleKeyYour2WaySend0, NumYour2WayElem[0], MPI_DOUBLE, myid - 1,
                         send_tag1 + myid - 1, MPI_COMM_WORLD, &(Request2WayRecv[0]));
        
        ierr = MPI_Isend((void *) DoubleKeyMy2WaySend0, NumMy2WayElem, MPI_DOUBLE, myid - 1, send_tag1 + myid,
    ...
    if(If2WaySend[1])
    {
        ...
        ierr = MPI_Irecv((void *) DoubleKeyYour2WaySend1, NumYour2WayElem[1], MPI_DOUBLE, myid + 1,
                         send_tag1 + myid + 1, MPI_COMM_WORLD, &(Request2WayRecv[1]));
        
        ierr = MPI_Isend((void *) DoubleKeyMy2WaySend1, NumMy2WayElem, MPI_DOUBLE, myid + 1, send_tag1 + myid,
            ...
            if(If2WaySend[0])
            {
                ...
                if(IfSentRecvd)
                {
                    ...
                    if(timeprops_ptr->iter == DEBUG_ITER)
                    {
                        ...
                        fprintf(fpdebug,
                                "myid=%d SequentialSend() EE If2WaySend[0]=%d ime=%d iyou=%d\n  (num_elem      -1)/2=%6d num_send1[0]=%6d num_send2[0]=%6d num_send[0]=%6d firstdk=%20.14g\n  (num_neigh_elem-1)/2=%6d num_recv1[0]=%6d num_recv2[0]=%6d num_recv[0]=%6d lastdk =%20.14g\n",
                                myid, If2WaySend[0], ime, iyou, (num_elem - 1) / 2, num_send1[0], num_send2[0],
                                num_send[0], DoubleKeyMy2WaySend0[0], (num_neigh_elem[0] - 1) / 2, num_recv1[0],
                                num_recv2[0], num_recv[0], DoubleKeyYour2WaySend0[(num_neigh_elem[0] - 1) / 2 - 1]);
            ...
            if(If2WaySend[1])
            {
                ...
                if(IfSentRecvd)
                {
                    ...
                    if(timeprops_ptr->iter == DEBUG_ITER)
                    {
                        ...
                        fprintf(fpdebug,
                                "myid=%d SequentialSend() FF If2WaySend[1]=%d ime=%d iyou=%d\n  (num_elem      -1)/2=%6d num_send1[1]=%6d num_send2[1]=%6d num_send[1]=%6d lastdk =%20.14g\n  (num_neigh_elem-1)/2=%6d num_recv1[1]=%6d num_recv2[1]=%6d num_recv[1]=%6d firstdk=%20.14g\n",
                                myid, If2WaySend[1], ime, iyou, (num_elem - 1) / 2, num_send1[1], num_send2[1],
                                num_send[1], DoubleKeyMy2WaySend1[(num_elem - 1) / 2 - 1], (num_neigh_elem[1] - 1) / 2,
                                num_recv1[1], num_recv2[1], num_recv[1], DoubleKeyYour2WaySend1[0]);
    ...
    if(timeprops_ptr->iter == DEBUG_ITER)
    {
        ...
        fprintf(fpdb2,
                "myid=%d repart2 num_elem=%d\n   num_send={%d,%d} num_send1={%d,%d} num_send2={%d,%d}\n   num_recv={%d,%d} num_recv1={%d,%d} num_recv2={%d,%d}\n",
                myid, num_elem, num_send[0], num_send[1], num_send1[0], num_send1[1], num_send2[0], num_send2[1],
                num_recv[0], num_recv[1], num_recv1[0], num_recv1[1], num_recv2[0], num_recv2[1]);
        fclose(fpdb2);
    ...
    if(num_send[0] > 0)
    {
        ...
        SET_OLDKEY( (MyFirstAndLastKey+0 * KEYLENGTH), ElemArray[isend[0] + 1]->key());
        
        for(ielem = 0; ielem <= isend[0]; ielem++)
        {
            Pack_element(ElemArray[ielem], send_array0 + ielem, NodeTable, myid - 1);
            assert(!(sfc_key_from_oldkey(send_array0[ielem].key)==SFC_Key(0)));
        }
        ...
        if(timeprops_ptr->iter == DEBUG_ITER)
        {
            ...
            if(num_send[0] == 0)
                fprintf(fpdb2, "myid=%d yadaA isend[0]=%d num_send[0]=%d num_elem=%d ielem=%d\n", myid, isend[0],
                        num_send[0], num_elem, ielem);
            else
                fprintf(fpdb2, "myid=%d yadaA isend[0]=%d num_send[0]=%d num_elem=%d ielem=%d {%10u,%10u}\n", myid,
                        isend[0], num_send[0], num_elem, ielem, send_array0[isend[0]].key[0],
                        send_array0[isend[0]].key[1]);
        ...
        ierr = MPI_Isend((void*) send_array0, num_send[0], ELEMTYPE, myid - 1, send_tag2 + myid, MPI_COMM_WORLD,
                         requestsent + 0);
    }
    ...
    if(num_recv[0] > 0)
    {
        ...
        ierr = MPI_Irecv((void *) recv_array0, num_recv[0], ELEMTYPE, myid - 1, send_tag2 + myid - 1, MPI_COMM_WORLD,
                         requestrecv + 0);
    ...
    if(num_send[1] > 0)
    {
        ...
        SET_OLDKEY((MyFirstAndLastKey+1 * KEYLENGTH),ElemArray[isend[1] - 1]->key());
        
        for(ielem = isend[1]; ielem < num_elem; ielem++)
        {
            Pack_element(ElemArray[ielem], send_array1 + ielem - isend[1], NodeTable, myid + 1);
            assert(!(sfc_key_from_oldkey(send_array1[ielem - isend[1]].key)==SFC_Key(0)));
        }
        ...
        if(timeprops_ptr->iter == DEBUG_ITER)
        {
            ...
            if(num_send[1] == 0)
                fprintf(fpdb2, "myid=%d yadaB isend[1]=%d num_send[1]=%d num_elem=%d ielem=%d\n", myid, isend[1],
                        num_send[1], num_elem, ielem);
            else
                fprintf(fpdb2, "myid=%d yadaB isend[1]=%d num_send[1]=%d num_elem=%d ielem=%d {%10u,%10u}\n", myid,
                        isend[1], num_send[1], num_elem, ielem, send_array1[0].key[0], send_array1[0].key[1]);
        ...
        ierr = MPI_Isend((void*) send_array1, num_send[1], ELEMTYPE, myid + 1, send_tag2 + myid, MPI_COMM_WORLD,
                         requestsent + 1);
    }
    ...
    if(num_recv[1])
    {
        ...
        ierr = MPI_Irecv((void *) recv_array1, num_recv[1], ELEMTYPE, myid + 1, send_tag2 + myid + 1, MPI_COMM_WORLD,
                         requestrecv + 1);
    ...
    if(timeprops_ptr->iter == DEBUG_ITER)
    {
        ...
        fprintf(fpdb2, "myid=%d B) MyFirstAndLastKey={{%10u,%10u},{%10u,%10u}}\n", myid, MyFirstAndLastKey[0],
                MyFirstAndLastKey[1], MyFirstAndLastKey[2], MyFirstAndLastKey[3]);
        fflush(stdout);
    ...
    if(timeprops_ptr->iter == DEBUG_ITER)
    {
        ...
        for(ielem = 0; ielem <= isend[0]; ielem++)
        {
            ...
            fprintf_sfc_key(fpdb2, ElemArray[ielem]->key());
            fprintf(fpdb2, "}\n");
        }
        ...
        if(num_recv[0] > 0)
        {
            ...
            if(IfSentRecvd)
            {
                ...
                if(timeprops_ptr->iter == DEBUG_ITER)
                {
                    ...
                    fprintf(fpdb2,
                            "myid=%d repart2 at 17.1... received \"sequential send\" elements from processor %d\n",
                            myid, myid - 1);
                    fclose(fpdb2);
                }
#endif
                ...
                if(timeprops_ptr->iter == DEBUG_ITER)
                {
                    ...
                    fprintf(fpdb2,
                            "myid=%d sender id=%d num_recv[0]=%d IncomingFirstLastKey={{%10u,%10u} {%10u,%10u}}\n",
                            myid, myid - 1, num_recv[0], recv_array0[0].key[0], recv_array0[0].key[1],
                            recv_array0[num_recv[0] - 1].key[0], recv_array0[num_recv[0] - 1].key[1]);
                    fclose(fpdb2);
                }
#endif
        ...
        if(num_recv[1] > 0)
        {
            ...
            if(IfSentRecvd)
            {
                ...
                if(timeprops_ptr->iter == DEBUG_ITER)
                {
                    ...
                    fprintf(fpdb2,
                            "myid=%d repart2 at 17.3... received \"sequential send\" elements from processor %d\n",
                            myid, myid + 1);
                    fclose(fpdb2);
                }
#endif
                ...
                if(timeprops_ptr->iter == DEBUG_ITER)
                {
                    ...
                    fprintf(fpdb2,
                            "myid=%d sender id=%d num_recv[1]=%d IncomingFirstLastKey={{%10u,%10u} {%10u,%10u}}\n",
                            myid, myid + 1, num_recv[1], recv_array1[0].key[0], recv_array1[0].key[1],
                            recv_array1[num_recv[1] - 1].key[0], recv_array1[num_recv[1] - 1].key[1]);
                    fclose(fpdb2);
                }
#endif
                doublekey = recv_array1[num_recv[1] - 1].key[0];
                for(ikey = 1; ikey < KEYLENGTH; ikey++)
                    doublekey = doublekey * doublekeyrange1 + recv_array1[num_recv[1] - 1].key[ikey];
                
                if(AllFirstAndLastDoubleKeys[myid][1] < doublekey)
                    //my last key has changed, update my record
                    for(ikey = 0; ikey < KEYLENGTH; ikey++)
                        MyFirstAndLastKey[1 * KEYLENGTH + ikey] = recv_array1[num_recv[1] - 1].key[ikey];
    ...
    for(ikey = 1; ikey < KEYLENGTH; ikey++)
    {
        MyFirstAndLastDoubleKey[0] = MyFirstAndLastDoubleKey[0] * doublekeyrange1 + MyFirstAndLastKey[ikey];
        MyFirstAndLastDoubleKey[1] = MyFirstAndLastDoubleKey[1] * doublekeyrange1 + MyFirstAndLastKey[KEYLENGTH + ikey];
    }
    ...
    if(myid == numprocs - 1)
    {
        ...
        for(ikey = 1; ikey < KEYLENGTH; ikey++)
            MyFirstAndLastDoubleKey[1] = MyFirstAndLastDoubleKey[1] * doublekeyrange1 + doublekeyrange1;
    }
    
    //double **AllFirstAndLastDoubleKeys=CAllocD2(numprocs,2);
    
    MPI_Allgather(MyFirstAndLastDoubleKey, 2, MPI_DOUBLE, *AllFirstAndLastDoubleKeys, 2, MPI_DOUBLE, MPI_COMM_WORLD);
    ...
    for(iproc = 0; iproc < numprocs; iproc++)
    {
        if(AllFirstAndLastDoubleKeys[iproc][0] > AllFirstAndLastDoubleKeys[iproc][1])
        {
            ifrepeat = 1;
            break;
        }
        for(jproc = iproc + 1; jproc < numprocs; jproc++)
            if((AllFirstAndLastDoubleKeys[iproc][0] > AllFirstAndLastDoubleKeys[jproc][0]) || (AllFirstAndLastDoubleKeys[iproc][1]
    ...
    if(!ifrepeat)
        for(iproc = 0; iproc < numprocs; iproc++)
            NewProcDoubleKeyBoundaries[iproc + 1] = AllFirstAndLastDoubleKeys[iproc][1];
                                     ...
                                     TimeProps* timeprops_ptr, double *NewProcDoubleKeyBoundaries)
{
    ...
    MyFirstAndLastDoubleKey[0] = NewProcDoubleKeyBoundaries[myid];
    MyFirstAndLastDoubleKey[1] = NewProcDoubleKeyBoundaries[myid + 1];
    ...
    for(ibuck = 0; ibuck < num_buck; ibuck++)
    {
        ...
        while (currentPtr)
        {
               ...
               && (EmTemp->adapted_flag() >= -BUFFER))
            {
                ...
                fpdb2 = fopen(fname2, "a");
                fprintf(fpdb2,
                        "myid=%d iter=%d MyFLDK={%20.14g,%20.14g} NPDKB={%20.14g,%20.14g} elemdoublekey=%20.14g\n",
                        myid, timeprops_ptr->iter, MyFirstAndLastDoubleKey[0], MyFirstAndLastDoubleKey[1],
                        NewProcDoubleKeyBoundaries[myid], NewProcDoubleKeyBoundaries[myid + 1], doublekey);
                fclose(fpdb2);
            }
#endif     
            ...
            if(EmTemp->adapted_flag() >= NOTRECADAPTED)
            {
                ...
                if((doublekey < MyFirstAndLastDoubleKey[0]) || (doublekey > MyFirstAndLastDoubleKey[1]))
                {
    ...
    if(timeprops_ptr->iter == DEBUG_ITER)
    {
        ...
        fprintf(fpdb2, "myid=%d iter=%d\n", myid, timeprops_ptr->iter);
        for(iproc = 0; iproc < numprocs; iproc++)
            fprintf(fpdb2, "     to/from iproc=%4d  #send=%5d #recv=%5d\n", iproc, NumToSecondSend[iproc],
                    NumToSecondRecv[iproc]);
        fclose(fpdb2);
    }
#endif
    ...
    for(iproc = 0; iproc < numprocs; iproc++)
    {
        if(NumToSecondRecv[iproc] > 0)
        {
            ...
            //if I'm not going to second-receive from them I'm already done 
    ...
    for(ielem = 0; ielem < NotMyElem.get_num_elem(); ielem++)
    {
        ...
        for(iproc = 0; iproc < numprocs; iproc++)
            if((NewProcDoubleKeyBoundaries[iproc] < doublekey) && (NewProcDoubleKeyBoundaries[iproc + 1] >= doublekey))
            {
                assert(NumToSecondSend[iproc] > IfSecondSendDone[iproc]);
                Pack_element(NotMyElem.get(ielem), SecondSendArray[iproc] + IfSecondSendDone[iproc], NodeTable, iproc);
    ...
    for(iproc = 0; iproc < numprocs; iproc++)
    {
        ...
        if(timeprops_ptr->iter == DEBUG_ITER)
        {
            ...
            fprintf(fpdb2, "myid=%d iter=%d iproc=%d IfSecondSendDone=%d NumToSecondSend=%d NumNotMyElem=%d\n", myid,
                    timeprops_ptr->iter, iproc, IfSecondSendDone[iproc], NumToSecondSend[iproc],
                    NotMyElem.get_num_elem());
            fclose(fpdb2);
        }
#endif
        ...
        if(NumToSecondSend[iproc] > 0)
        {
            ...
            for(iSecondSend = 0; iSecondSend < NumToSecondSend[iproc]; iSecondSend++)
                assert(!(sfc_key_from_oldkey(SecondSendArray[iproc][iSecondSend].key)==sfc_key_null));
            
            //non blocking second-send to processor iproc
            ierr = MPI_Isend((void *) SecondSendArray[iproc], NumToSecondSend[iproc], ELEMTYPE, iproc, send_tag3 + myid,
                             MPI_COMM_WORLD, &(RequestSecondSend[iproc]));
#ifdef DEBUG_REPART2C
            if(timeprops_ptr->iter == DEBUG_ITER)
            {
                fpdb2 = fopen(fname2, "a");
    ...
    do
    {
        ...
        for(iproc = 0; iproc < numprocs; iproc++)
            if(!IfSecondRecvDone[iproc])
            {
                ...
                if(IfSentRecvd)
                {
                    IncorporateNewElements(El_Table, NodeTable, myid, NumToSecondRecv[iproc], SecondRecvArray[iproc],
                                           timeprops_ptr);
                    IfSecondRecvDone[iproc] = 1;
