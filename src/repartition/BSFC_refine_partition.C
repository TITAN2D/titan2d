	...
	*local_balanced_flag = BSFC_BALANCED;
	...
	for (ll_counter = 0; ll_counter <= number_of_cuts; ll_counter++)
		if ((local_balanced_flag_array[ll_counter] == BSFC_NOT_BALANCED)
			...
			while (ll_location != -1) {
				vert_in_cut_ptr[ll_location].my_bin = BSFC_get_array_location(number_of_bins,
				    amount_of_bits, *amount_of_used_bits, (vert_in_cut_ptr + ll_location));
			...
			while (ll_location != -1) {
				if (bin_proc_array[vert_in_cut_ptr[ll_location].my_bin] >= 0) {
					...
					if (ll_bins_head_copy[number_of_cuts - myid + //array continued on the next line
						...
						vert_in_cut_ptr[ll_location].next_sfc_vert_index = ll_bins_head_copy[number_of_cuts
						    - myid + vert_in_cut_ptr[ll_location].destination_proc];
	...
	*amount_of_used_bits += amount_of_bits;
	...
	for (i = 0; i <= number_of_cuts; i++)
		if (ll_bins_head[i] != -1 || local_balanced_flag_array[i] != BSFC_BALANCED)
			local_balanced_flag_array[i] = BSFC_find_imbalance(work_percent_array,
			    global_actual_work_allocated[(myid + i - number_of_cuts)], total_weight,
