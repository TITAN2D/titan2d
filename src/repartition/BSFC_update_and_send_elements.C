...
void BSFC_update_and_send_elements(int myid, int numprocs, HashTable* HT_Elem_Ptr,
	...
	for (i = 0; i < numprocs; i++) {
		if (recv_info[2 * i] != 0) {  // receive neighbor info
			j = MPI_Irecv((recv_neigh_array + counter_recv[0]), recv_info[2 * i] * (2 * KEYLENGTH + 1),
			    MPI_UNSIGNED, i, neigh_tag, MPI_COMM_WORLD, (recv_request + i * 2));
	...
	for (i = 0; i < numprocs; i++)
		if (send_info[2 * i] != 0) {
			j = MPI_Isend((send_neigh_array + counter * (2 * KEYLENGTH + 1)),
			    send_info[2 * i] * (2 * KEYLENGTH + 1), MPI_UNSIGNED, i, neigh_tag, MPI_COMM_WORLD,
			    (send_request + i));
				...
				if (myid != EmTemp->get_myprocess()) { // this element will get moved to a new processor
					...
					for (j = 0; j < 8; j++) {
						if (neigh_proc[j] == myid) {
							Element* EmNeigh = (Element*) HT_Elem_Ptr->lookup(
							    (EmTemp->get_neighbors() + j * KEYLENGTH));
...
// wait to receive new neighbor info and then update neighbor info
		...
		while (entryp) {
			...
			if (!EmTemp->get_refined_flag()) {
				if (myid != EmTemp->get_myprocess()) { // this element will get moved to a new processor
					...
					Pack_element(EmTemp, (send_elm_array + counter_send_proc[myprocess]), HT_Node_Ptr,
					    myprocess);
	...
	for (i = 0; i < numprocs; i++)
		if (send_info[2 * i + 1] != 0) {
			j = MPI_Isend((send_elm_array + counter), send_info[2 * i + 1], ELEMTYPE, i, elm_tag,
