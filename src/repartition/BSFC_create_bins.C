 ...
 *
 *******************************************************************
 * $Id: BSFC_create_bins.C 224 2011-12-04 20:49:23Z dkumar $ 
 */
...
void BSFC_create_bins(int num_local_objects, BSFC_VERTEX_PTR sfc_vert_ptr, int* amount_of_bits_used,
	...
	*amount_of_bits_used = amount_of_bits;
	...
	for (i = 0; i < num_local_objects; i++) {
		sfc_vert_ptr[i].my_bin = BSFC_get_array_location(number_of_bins, amount_of_bits, 0,
		    (sfc_vert_ptr + i));
	...
	i = MPI_Allreduce(tmp_float_array, binned_weight_array, number_of_bins + 1, MPI_FLOAT, MPI_SUM,
	    MPI_COMM_WORLD);
	...
	*total_weight_ptr = binned_weight_array[number_of_bins];
	...
	local_balanced_flag = BSFC_find_imbalance(work_percent_array, global_actual_work_allocated[myid],
	...
	i = MPI_Alltoall(verts_in_cut_info->send_procs_ptr, 1, MPI_INT, verts_in_cut_info->recv_procs_ptr,
	...
	for (i = 0; i < numprocs; i++) {
		if (i != myid) {
			...
			if (verts_in_cut_info->send_procs_ptr[i] != 0) {
				j = MPI_Isend((send_sfc_vert + proc_counter[i]), verts_in_cut_info->send_procs_ptr[i],
...
int BSFC_get_array_location(int number_of_bins, int number_of_bits, int prev_used_bits,
...
/* routine finds the imbalance for processor which_proc given the
 ...
 tolerance.   */
int BSFC_find_imbalance(float* work_percent_array, float cumulative_work, float total_work,
    int which_proc, int numprocs)
	...
	if (which_proc != numprocs - 1)
		my_ideal_work = (work_percent_array[which_proc] - work_percent_array[which_proc + 1])
		    * total_work;
	else
