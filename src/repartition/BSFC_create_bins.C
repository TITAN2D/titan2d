                      ...
                      int* number_of_cuts, int bins_per_proc, int myid, int numprocs)
{
    ...
    *amount_of_bits_used = amount_of_bits;
    ...
    {
        sfc_vert_ptr[i].my_bin = BSFC_get_array_location(number_of_bins, amount_of_bits, 0, (sfc_vert_ptr + i));
    ...
    *total_weight_ptr = binned_weight_array[number_of_bins];
    ...
    for(i = 0; i < numprocs; i++)
    {
        if(i != myid)
        {
            ...
            {
                j = MPI_Isend((send_sfc_vert + proc_counter[i]), verts_in_cut_info->send_procs_ptr[i], LB_VERT_TYPE, i,
                              tag, MPI_COMM_WORLD, (send_request + i));
            }
            // receive necessary info here...
            if(verts_in_cut_info->recv_procs_ptr[i] != 0)
            {
                j = MPI_Irecv(&(verts_in_cut_info->recv_sfc_vert[recv_count]), verts_in_cut_info->recv_procs_ptr[i],
...
/* NOTE:  which_proc is not required to be this procs rank */
{
    ...
    if(which_proc != numprocs - 1)
        my_ideal_work = (work_percent_array[which_proc] - work_percent_array[which_proc + 1]) * total_work;
    else
